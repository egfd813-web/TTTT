...
def is_valid_checkin_time(shift: str, now: datetime = None) -> tuple[bool, str]:
    ...
    if shift == "1":      
        if current_time < datetime_time(14, 0):
            return False, "⚠️ 第一班上班需在 **14:00之后** 打卡"
    elif shift == "2":    
        if current_time >= datetime_time(21, 0):
            return False, "⚠️ 第一班下班需在 **21:00之前** 打卡"
    elif shift == "3":    
        if current_time < datetime_time(21, 0):
            return False, "⚠️ 第二班上班需在 **21:00之后** 打卡"
    elif shift == "4":    
        if current_time >= datetime_time(6, 0):
            return False, "⚠️ 第二班下班需在 **06:00之前** 打卡（05:00-06:00）"
...
def is_valid_rest_time(shift: str) -> tuple[bool, str]:
    ...
    if (datetime_time(14, 30) <= current_time < datetime_time(20, 0)) or \
       (datetime_time(19, 30) <= current_time) or \
       (current_time < datetime_time(6, 0)):
        return True, ""
...
def get_report_date_for_daily(now=None) -> str:
    """自动日报专用 - 确保取前一天完整考勤日期 (基于07:00分界点)"""
    if now is None:
        now = beijing_now()
    # 在 07:00 之前触发日报，应该生成前一天的报表
    # 如果当前时间是 06:30，那么 get_attendance_date(now) 会返回前一天
    # 所以直接返回 get_attendance_date(now) 即可，它会根据 07:00 分界点自动处理
    return get_attendance_date(now)
...
# ================== ACTIONS ==================
ACTIONS = {
    "1": {"name": "第一班上班", "time": "15:00", "is_work": True,  "type": "work"},
    "2": {"name": "第一班下班", "time": "20:00", "is_work": False, "type": "work"},
    "3": {"name": "第二班上班", "time": "22:00", "is_work": True,  "type": "work"},
    "4": {"name": "下班打卡完成，请将设备摆放整齐并开启飞行模式", "time": "05:00", "is_work": False, "type": "work"},
...
def main():
    ...
    daily_time = datetime_time(6, 30, 0, tzinfo=beijing_tz)
    cleanup_time = datetime_time(6, 40, 0, tzinfo=beijing_tz)
